...
function Eslinkgroup (clazz, options, mid) {
  ...
  this.links = options.map(function (option) {
    return new Eslink(clazz, option, mid, color)
  })
...
Eslink.prototype.nearby = function (other) {
  ...
  var r = this.option.getBoundingClientRect()
  var rr = other.option.getBoundingClientRect()
...
Espath.prototype.addpoint = function (x, y) {
  this.points.push({ x: round(x), y: round(y) })
  return this
}
...
function reorder (leftEslinks) {
  ...
  var visiblehelp = $('#help pre:visible')
  ...
  // order
  if (
    visiblehelp.index($(help[0])) >= visiblehelp.index($(help[help.length - 1]))
  ) {
    return
  }

  for (
    var i = 0, j = help.length - 1;
...
function optionsselector (pres, spans) {
  ...
  var r = _.reduce(
    ids,
    function (s, id) {
      return s.add('#command span[helpref^=' + id + ']')
    },
    s
  )
...
function initialize () {
  ...
  while (curr) {
    if (curr.name != 'all') {
      $('span[class^=expansion]', curr.commandselector)
        .each(function () {
          var kind = $(this)
            .attr('class')
            .slice(10)
...
function drawgrouplines (commandselector, options) {
  ...
  if (helpselector(commandselector).length > 0) {
    ...
    $('#canvas').height(
      $('#help pre:visible')
        .last()[0]
        .getBoundingClientRect().bottom - canvasTop
    )
  ...
  var mid = commandWrapperRect.left + commandWrapperRect.width / 2

  var helprect = $('#help')[0].getBoundingClientRect()

  // the bounds of the area we plan to draw lines in .help
  var top = helprect.top - toppadding

  var left = helprect.left - sidepadding

  var right = helprect.right + sidepadding

  var topheight = options.topheight
  ...
  var groupedoptions = _.groupBy(
    commandselector.filter(':not(.unknown)'),
    function (span) {
      return $(span).attr('helpref')
    }
  )
  ...
  var marginBetweenCommandAndCanvas = commandWrapperRect.bottom - canvasTop

  var startytop = commandWrapperRect.top - canvasTop
  ...
  var l = _.filter(links, function (l) {
    return l.goingleft
  })
  ...
  for (var i = 0; i < links.length; i++) {
    ...
    var spanmid = commandRect.left + commandRect.width / 2

    var commandRight = commandRect.right - strokewidth
    ...
    if (link.goingleft) {
      ...
      if (link === leftmost) {
        ...
        let startyDifferace = leftmost.starty - link.starty
  ...
  var unknowngroup = new Eslinkgroup(
    null,
  ...
  $.each(unknowngroup.links, function (i, link) {
    ...
    var rrright = rr.right - strokewidth

    var nextspan = link.option.nextElementSibling

    var nextlink = _.find(links, function (l) {
      return l.option === nextspan
    })

    var prevspan = link.option.previousElementSibling

    var prevlink = _.find(links, function (l) {
      return l.option === prevspan
    })
    ...
    //    var rrmid = round(rr.left + rr.width / 2);
  ...
  if (linkslengthnounknown > 1) {
    var groupsnounknowns = groups.filter(function (g) {
      return !g.links[0].unknown
    })
    groupsnounknowns.each(function (linkgroup) {
      var othergroups = groups.filter(function (other) {
        return linkgroup != other
      })
      ...
      s.hover(
        function () {
          ...
          // be in there
          $('span[class$=substitution]', linkgroup.options).css({
            'font-weight': 'normal'
          })
          // and disable transparency
          $(linkgroup.help)
            .add(linkgroup.options)
            .css({ opacity: 1.0 })
...
function navigation () {
  ...
  if (currentgroup.next) {
    ...
    var currentext = prevnext.find('u')
...
function inview (viewtop, viewbottom, $el) {
  ...
  var elemmiddle = elemtop + $el.height() / 2

  var elemarea = $el.width() * $el.height()
...
function drawvisible () {
  ...
  if (visible.length > 0) {
    ...
    var commandselector = optionsselector(
      visible,
